...
import { productoController } from '../../di/container'
...
export default function ProductosList() {
  ...
  const cargar = async () => {
    ...
    setError('')
    try {
      setProductos(await productoController.listar())
    } catch (err) {
  ...
  const handleCrear = async (datos: NuevoProducto) => {
    await productoController.crear(datos)
    setMostrarForm(false)
    await cargar()
  }

  const handleActualizar = async (datos: NuevoProducto) => {
    if (!editando) return
    await productoController.actualizar(editando.id, datos)
    setEditando(null)
    await cargar()
  }
